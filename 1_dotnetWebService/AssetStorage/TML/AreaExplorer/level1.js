...
function PageRefresh(userData) {
    ...
    $(`#AreaExploreTitle`).append(
        `<h6 class="text-white text-capitalize ps-3">${getplant ? getplant : plantName
    ...
    getCall.onSuccess = function onSuccess(result) {
        ...
        Dayarr.forEach((ele, index) => {
            $("#days_id").append(
                `<option value="${ele}">${ele == "1D" ? `${ele}ay` : `${ele}ays`
                }</option>`
            );
        });
...
function display_alarm(obj, index, i) {
    ...
    var machine_linedata = `<div class="col-sm-12">
                                    <div class="line-layout row">
                                      <span class="line-machine col-sm-4 px-0">${obj.component
        }</span>
        ...
        }" style="width:${okavg ? Math.round(okavg) : 0
        }%;">${obj.ok}</span>
                                        <span class="split-red" title="${obj.nok
        }" style="width:${nokavg ? Math.round(nokavg) : 0
        }%;">${obj.nok}</span>
                                      </div>
                                    </div>
                                    </div>
